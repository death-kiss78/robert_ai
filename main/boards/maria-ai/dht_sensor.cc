...
DhtSensor::DhtSensor(gpio_num_t pin) : dht_(pin) {
    auto& mcp = McpServer::GetInstance();

    // 🔥 TOOL PRINCIPAL
    mcp.AddTool("self.sensor.dht.read", "Read temperature and humidity from DHT sensor",
                PropertyList(), [this](const PropertyList&) -> ReturnValue {
                    ...
                    if (!ok) {
                        if (dht_.IsDataFresh(30000)) {
                            ...
                            snprintf(buf, sizeof(buf),
                                     ...
                                     (unsigned long)dht_.GetDataFreshness(), dht_.GetTemperature(),
                                     dht_.GetHumidity());
                            return buf;
                        }
                    ...
                    char buf[64];
                    snprintf(buf, sizeof(buf), "{\"temperature\": %d, \"humidity\": %d}",
                             dht_.GetTemperature(), dht_.GetHumidity());
    ...
    // 🔥 TOOL DE TEST
    mcp.AddTool(
        "self.sensor.dht.test", "Test if the DHT sensor is working properly", PropertyList(),
        [this](const PropertyList&) -> ReturnValue {
            ...
            if (ok) {
                ...
                ESP_LOGI(TAG, "DHT test success! Temp: %d°C, Humidity: %d%%", temp, hum);

                char buf[128];
                snprintf(buf, sizeof(buf), "{\"ok\": true, \"temperature\": %d, \"humidity\": %d}",
                         temp, hum);
                return buf;
            }
    ...
    xTaskCreatePinnedToCore(BackgroundTask, "dht_bg_task", 4096, this, 5, nullptr, 0);
}
